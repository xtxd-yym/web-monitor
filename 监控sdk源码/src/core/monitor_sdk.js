...
class DynamicWebMonitor {
  constructor(options = {}) {
    ...
    this.cacheKey = 'web_monitor_config';
    this.isClosed = false;
  }

  // 静态变量：全局请求去重
  static pendingConfigRequest = null;
  ...
  async getConfig() {
    ...
    // ===== 优化2: 请求去重 =====
    if (DynamicWebMonitor.pendingConfigRequest) {
      console.log('[Config] Reusing pending request');
      return await DynamicWebMonitor.pendingConfigRequest;
    }

    // 发起新请求
    console.log('[Config] Fetching from server...');
    DynamicWebMonitor.pendingConfigRequest = (async () => {
      try {
        ...
        DynamicWebMonitor.pendingConfigRequest = null;
      }
    })();
  ...
  async init() {
    try {
      ...
      if (!dynamicConfig.enabled) {
        console.log('Monitoring disabled by dynamic configuration, using fallback defaults');
        dynamicConfig = {
          enabled: true,
          config: {
            // 性能上报：关
